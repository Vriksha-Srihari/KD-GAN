...
def sent_loss(cnn_code, rnn_code, labels, class_ids,
    ...
    if cnn_code.dim() == 2:
        cnn_code = cnn_code.unsqueeze(0)  # (1,N,nef)
        rnn_code = rnn_code.unsqueeze(0)  # (1,N,nef)

    # cnn_code_norm / rnn_code_norm: seq_len x batch_size x 1
    cnn_code_norm = torch.norm(cnn_code, 2, dim=2, keepdim=True)
    rnn_code_norm = torch.norm(rnn_code, 2, dim=2, keepdim=True)
    # scores* / norm*: seq_len x batch_size x batch_size
    scores0 = torch.bmm(cnn_code, rnn_code.transpose(1, 2))          # (1,N,N)
    norm0 = torch.bmm(cnn_code_norm, rnn_code_norm.transpose(1, 2))  # (1,N,N)
    scores0 = scores0 / norm0.clamp(min=eps) * cfg.TRAIN.SMOOTH.GAMMA3

    # --> batch_size x batch_size
    scores0 = scores0.squeeze()  # (N,N)
    if class_ids is not None:
...
def generator_loss(netsD, image_encoder, fake_imgs, real_labels,
    ...
    for i in range(numDs):
        ...
        if i == (numDs - 1):
            ...
            log_info['w_loss'] = '%.4f' % w_loss.item()
            log_info['s_loss'] = '%.4f' % s_loss.item()
    ...
    return errG_total, logs, log_info
