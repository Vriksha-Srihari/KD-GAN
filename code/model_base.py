...
class RNN_ENCODER(nn.Module):
    def __init__(self, ix2vec, ninput=300, drop_prob=0.5,
    ...
    def captions_to_vec(self, captions):
        batch, T = captions.size()
        emb_dim = 300
        if cfg.CUDA:
            captions_new = torch.zeros([batch, T, emb_dim]).cuda()
        else:
            captions_new = torch.zeros([batch, T, emb_dim])
        for i in range(batch):
            cap = captions[i]
            for j in range(T):
                captions_new[i,j] = self.ix2vec[cap[j]]
    ...
    def forward(self, captions, cap_lens, hidden, mask=None):
        ...
        emb = self.captions_to_vec(captions)
